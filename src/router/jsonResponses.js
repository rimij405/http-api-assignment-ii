...
const sendResponseJSON = (request, response, statusCode, object) => {
  router.sendResponse(request, response, statusCode, mimetype.JSON, JSON.stringify(object));
};

// Get meta version of JSON response.
const sendResponseJSONMeta = (request, response, statusCode) => {
  router.sendResponseMeta(request, response, statusCode, mimetype.JSON);
};
